...
Scopes、Breakpoints、Stacks、Watches 停靠在左侧，REPL 和程序控制台与终端、
quickfix 一起排在底部。位置由 edgy 决定，和这里其他面板一视同仁，所以 `so`
和窗口键对待它们的方式与 AI 面板相同，而不是另起一套布局。一个适配器驱动多个
目标时，面板只打开一次，不会每个目标打开一次。
