...
| `g` | 去某处 / 关于光标下的东西 | `gd` 定义 · `gr` 引用 · `gb` 实现 · `gy` 类型 · `gC` 调用方 · `K` 文档 · `gx` 打开 URL · `gS` 拆 / 合行 · `gv` 重选 |
...
是几个区块而不是一串字母。分组在 `lua/whichkey_spec.lua` 里声明；没有归类的键
排在最后。

`gx` 打开当前行离光标最近的 HTTP(S) 链接，没有链接时打开当前文件。在终端
Normal 模式下，表格同一列中被换行拆开的 `(...)` 或 `<...>` 链接也会完整打开，
光标放在任一段都可以。先按 `Ctrl-\` 或 `jk` 退出终端输入模式。
